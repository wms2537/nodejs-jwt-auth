    ...
    stage('Deploy Images') {
      steps {
        sshagent(credentials:['ALICLOUD_HONG_KONG_SERVER_KEY']) {
            ...
            sh ('ssh -o StrictHostKeyChecking=no root@$SERVER_IP \"export BUILD_NUMBER=$BUILD_NUMBER; export DATABASE_URL=$MONGODB_WMTECH; cd /root/nodejs-jwt-auth/deploy && docker-compose up -d\"')
        }
      }
    }
  }
