    ...
    stage('Deploy Images') {
      steps {
        sshagent(credentials:['ALICLOUD_HONG_KONG_SERVER_KEY']) {
            ...
            sh ('ssh -o StrictHostKeyChecking=no root@$SERVER_IP \"BUILD_NUMBER=$BUILD_NUMBER DATABASE_URL=$MONGODB_WMTECH cd /root/nodejs-jwt-auth/deploy && docker-compose up -d --build\"')
        }
      }
    }
  }
